(($, window, document, undefined)=>{
  ...
  class Agency {
    ...
    header(){
      ...
      $('.sub').stop().slideUp(0); // 로딩시 초기화. 제일 바깥에 한번만 시작되면 된다.
    ...
    section1(){
      ...
      slideView.on({
        touchstart(event){ //터치시작 mousedown
           s = event.originalEvent.touches[0].clientX; // 모바일은 바닐라 자바스크립트
          dS = event.originalEvent.touches[0].clientX - slideWrap.offset().left-winW;
          mD = true;
        },
        touchend(event){ //터치종료 mouseup
          e = event.originalEvent.changedTouches[0].clientX; //여기는 끝나는 거니까 touches앞에 'changed'가 틀어가야 한다.
          mD = false;
          if( (s-e) > 0 ){  
            nextCount();
          }
